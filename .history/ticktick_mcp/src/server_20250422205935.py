...
def format_project(project: Dict) -> str:
    ...
    formatted += f"Kind: {project.get('kind', 'TASK')}\n"
    return formatted
...
async def get_projects() -> str:
    ...
    try:
        # If project_id is provided directly, just get that project
        if len(sys.argv) > 1 and sys.argv[1].startswith("project_"):
            project_id = sys.argv[1].split("_")[1]
